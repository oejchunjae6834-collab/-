...
export async function POST(req, { params }) {
  ...
  const fm = getFamilyMember(parseInt(family_member_id, 10));
  ...
  setSessionAttendance(sessionId, fm.id, status || null);

  // 실시간 합산 — 이 세션의 모든 가족 구성원 출석을 합산
  const all = listEventSessionAttendances(session.event_id).filter((r) => r.session_id === sessionId);
